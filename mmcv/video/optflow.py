...
def flowread(
    flow_or_path: Union[np.ndarray, str],
    quantize: bool = False,
    concat_axis: int = 0,
    *args,
    **kwargs,
...
def quantize_flow(flow: np.ndarray, max_val: float = 0.02, norm: bool = True) -> tuple:
...
def dequantize_flow(
    dx: np.ndarray, dy: np.ndarray, max_val: float = 0.02, denorm: bool = True
) -> np.ndarray:
