...
class FusedBiasLeakyReLUFunctionBackward(Function):
    ...
    This function is to compute the second order deviation for the fused leaky relu
    operation.
    """
    ...
    @staticmethod
    def backward(
        ctx, gradgrad_input: torch.Tensor, gradgrad_bias: nn.Parameter
    ) -> tuple:
...
class FusedBiasLeakyReLU(nn.Module):
    ...
    def __init__(
        self, num_channels: int, negative_slope: float = 0.2, scale: float = 2**0.5
    ):
        super().__init__()
