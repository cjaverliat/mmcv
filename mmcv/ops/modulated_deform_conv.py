...
class ModulatedDeformConv2dFunction(Function):
    ...
    def _npu_forward(ctx, input_tensor, offset, mask, weight, bias):
        ...
        output, offset_out = torch_npu.npu_deformable_conv2d(
            ...
            padding=[ctx.padding[0], ctx.padding[0], ctx.padding[1], ctx.padding[1]],
    ...
    def _npu_backward(ctx, grad_output):
        input_tensor, weight, offset_out, offset_all, sort_index_bp = ctx.saved_tensors
        import torch_npu
    ...
    def forward(
        ...
        output = input.new_empty(
            [
                int(i)
                for i in ModulatedDeformConv2dFunction._output_size(ctx, input, weight)
            ]
        )
    ...
    def backward(ctx, grad_output: torch.Tensor) -> tuple:
        if ctx.device == "npu":
            return ModulatedDeformConv2dFunction._npu_backward(ctx, grad_output)
