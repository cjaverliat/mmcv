...
def points_in_boxes_part(points: Tensor, boxes: Tensor) -> Tensor:
    ...
    assert boxes.shape[2] == 7, (
        "boxes dimension should be 7, " f"but got unexpected shape {boxes.shape[2]}"
    )
    assert points.shape[2] == 3, (
        "points dimension should be 3, " f"but got unexpected shape {points.shape[2]}"
    )
    ...
    assert (
        points_device == boxes.get_device()
...
def points_in_boxes_cpu(points: Tensor, boxes: Tensor) -> Tensor:
    ...
    point_indices = points.new_zeros(
        (batch_size, num_boxes, num_points), dtype=torch.int
    )
...
def points_in_boxes_all(points: Tensor, boxes: Tensor) -> Tensor:
    ...
    box_idxs_of_pts = points.new_zeros(
        (batch_size, num_points, num_boxes), dtype=torch.int
    ).fill_(0)
