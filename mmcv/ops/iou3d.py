...
def boxes_iou3d(boxes_a: Tensor, boxes_b: Tensor) -> Tensor:
    ...
    """
    assert (
        boxes_a.shape[1] == boxes_b.shape[1] == 7
...
def nms3d_normal(boxes: Tensor, scores: Tensor, iou_threshold: float) -> Tensor:
    """Normal 3D NMS function GPU implementation. The overlap of two boxes for IoU
    calculation is defined as the exact overlapping area of the two boxes WITH their yaw
    angle set to 0.
...
def nms_bev(
    ...
    warnings.warn(
        ...
        " the future. Please, use `nms.nms_rotated`.",
        DeprecationWarning,
    )
