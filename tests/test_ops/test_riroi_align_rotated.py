...
np_rois = np.array(
    [[0.0, 0.5, 0.5, 1.0, 1.0, np.pi / 3], [0.0, 1.0, 1.0, 3.0, 3.0, np.pi / 2]]
)
expect_output = np.array(
    [
        [
            [[1.8425, 1.3516], [2.3151, 1.8241]],
...
def test_roialign_rotated_gradcheck():
    ...
    froipool = RiRoIAlignRotated(
        (pool_h, pool_w), spatial_scale, num_samples, num_orientations, clockwise
    )
    if _USING_PARROTS:
        gradcheck(froipool, (x, rois), no_grads=[rois], delta=1e-3, pt_atol=1e-3)
    else:
