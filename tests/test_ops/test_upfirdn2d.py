...
class TestUpFirDn2d:
    ...
    Here, we just test the basic case of upsample version. More gerneal tests will be
    included in other unit test for UpFirDnUpsample and UpFirDnDownSample modules.
    """
    ...
    def test_upfirdn2d(self):
        ...
        if _USING_PARROTS:
            gradcheck(
                ...
                (
                    ...
                    self.kernel.type_as(self.input_tensor).cuda(),
                    self.factor,
                    1,
                    self.pad,
                ),
        ...
        out = upfirdn2d(self.input_tensor.cuda(), filter=kernel.cuda(), up=2, padding=1)
        ...
        out = upfirdn2d(
            self.input_tensor.cuda(), filter=self.kernel.cuda(), flip_filter=True
        )
        assert out.shape == (2, 3, 1, 1)

        # test with different gain
        out1 = upfirdn2d(self.input_tensor.cuda(), filter=self.kernel.cuda(), gain=0.2)
        out2 = upfirdn2d(self.input_tensor.cuda(), filter=self.kernel.cuda(), gain=0.1)
