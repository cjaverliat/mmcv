...
def test_roialign():
    ...
    for case in inputs:
        ...
        onnx_output = sess.run(
            None, {"input": input.detach().numpy(), "rois": rois.detach().numpy()}
        )
...
def test_roipool():
    ...
    def warpped_function(torch_input, torch_rois):
        return roi_pool(torch_input, torch_rois, (pool_w, pool_h), spatial_scale)
    ...
    for case in inputs:
        ...
        # compute pytorch_output
        with torch.no_grad():
            pytorch_output = roi_pool(input, rois, (pool_w, pool_h), spatial_scale)
...
def test_deform_conv():
    ...
    _test_symbolic(DeformConv2dPack(2, 4, 3, 1, 1).cuda(), (x,), "MMCVDeformConv2d")
...
def test_modulated_deform_conv():
    ...
    x = torch.randn(1, 2, 4, 4, device="cuda")
    _test_symbolic(
        ModulatedDeformConv2dPack(2, 4, 3, 1, 1).cuda(), x, "MMCVModulatedDeformConv2d"
    )
...
def test_deform_roi_pool():
    ...
    rois = torch.tensor([[0.0, 0.0, 0.0, 1.0, 1.0]], device="cuda")
    output_c = x.size(1)
    pool_h = 2
    pool_w = 2
    spatial_scale = 1.0
    sampling_ratio = 2
    model = DeformRoIPoolPack(
        (pool_h, pool_w),
        output_c,
...
def test_masked_conv():
    ...
    _test_symbolic(MaskedConv2d(2, 4, 3, 1, 1).cuda(), (x, mask), "MMCVMaskedConv2d")
...
def test_roi_align_rotated():
    ...
    sampling_ratio = 2
    x = torch.tensor([[[[1.0, 2.0], [3.0, 4.0]]]], device="cuda")
    rois = torch.tensor([[0.0, 0.5, 0.5, 1.0, 1.0, 0]], device="cuda")
    model = RoIAlignRotated((pool_h, pool_w), spatial_scale, sampling_ratio).cuda()
