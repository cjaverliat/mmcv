...
def test_dynamic_scatter(device):
    ...
    empty_o_feats = (
        torch.rand(size=(200000, 3), dtype=torch.float32, device=device) * 100 - 50
    )
    ...
    feats = torch.rand(size=(200000, 3), dtype=torch.float32, device=device) * 100 - 50
    coors = torch.randint(
    ...
    # test grad #
    feats = torch.rand(size=(100, 4), dtype=torch.float32, device=device) * 100 - 50
    coors = torch.randint(
