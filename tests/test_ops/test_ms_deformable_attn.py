...
def test_forward_equal_with_pytorch_npu():
    ...
    shapes = torch.as_tensor(
        [(60, 40), (30, 20), (16, 24), (53, 32)], dtype=torch.int32
    )
    level_start_index = torch.cat(
        (shapes.new_zeros((1,)), shapes.prod(1).cumsum(0)[:-1])
    )
    ...
    output_pytorch = (
        multi_scale_deformable_attn_pytorch(
            value.float(), shapes, sampling_locations.float(), attention_weights.float()
        )
        .detach()
        .cpu()
    )
    ...
    output_npu = (
        MultiScaleDeformableAttnFunction.apply(
            value.npu().float(),
            shapes.npu(),
            level_start_index.npu(),
            sampling_locations.npu().float(),
            attention_weights.npu().float(),
            im2col_step,
        )
        .detach()
        .cpu()
    ...
    max_rel_err = ((output_npu - output_pytorch).abs() / output_pytorch.abs()).max()
    assert max_abs_err < 1e-18
    assert max_rel_err < 1e-15
...
@pytest.mark.skipif(not _IS_AUTOCAST_AVAILABLE, reason="requires autocast support")
@pytest.mark.parametrize(
    "device",
    [
        pytest.param(
...
def test_forward_equal_with_autocast(device):
    ...
    assert torch.allclose(output_device, output_pytorch.half(), rtol=1e-2, atol=1e-3)
    max_abs_err = (output_device - output_pytorch).abs().max()
    max_rel_err = ((output_device - output_pytorch).abs() / output_pytorch.abs()).max()
...
def test_backward_equal_with_pytorch_npu():
    ...
    output_npu = MultiScaleDeformableAttnFunction.apply(
        ...
        sampling_locations_npu.float(),
        attention_weights_npu.float(),
        im2col_step,
    )
    ...
    max_rel_err_1 = ((grad_value_npu - grad_value).abs() / grad_value.abs()).max()
    ...
    max_rel_err_2 = (
        (grad_location_npu - grad_location).abs() / grad_location.abs()
    ).max()
    ...
    max_rel_err_3 = (
        (grad_attn_weight_npu - grad_attn_weight).abs() / grad_attn_weight.abs()
    ).max()
