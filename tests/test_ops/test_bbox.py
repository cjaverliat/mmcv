...
class TestBBox:
    ...
    def _test_bbox_overlaps(self, device="cpu", dtype=torch.float):
        ...
        b2 = (
            torch.tensor([[0.0, 2.0, 2.0, 5.0], [2.0, 1.0, 3.0, 3.0]])
            .to(device)
            .type(dtype)
        )
        ...
        b1 = (
            torch.tensor([[1.0, 1.0, 3.0, 4.0], [2.0, 2.0, 3.0, 4.0]])
            .to(device)
            .type(dtype)
        )
        b2 = (
        ...
        b1 = (
            torch.tensor(
                [[10.0 + i, 10.0 + i, 30.0 + i, 30.0 + i] for i in range(1000)]
            )
            .to(device)
            .type(dtype)
        )
        b2 = (
            torch.tensor(
                [[20.0 + i, 20.0 + i, 40.0 + i, 40.0 + i] for i in range(1000)]
            )
            .to(device)
            .type(dtype)
        )
