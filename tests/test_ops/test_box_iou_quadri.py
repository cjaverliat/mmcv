...
class TestBoxIoUQuadri:
    ...
    def test_box_iou_quadri_cuda(self, device):
        ...
        np_boxes1 = np.asarray(
            [
                [1.0, 1.0, 3.0, 4.0, 4.0, 4.0, 4.0, 1.0],
        ...
        np_expect_ious = np.asarray(
            [
                [0.0714, 1.0000, 0.0000],
                [0.0000, 0.5000, 0.0000],
                [0.0000, 0.0000, 0.5000],
            ],
            dtype=np.float32,
        )
        np_expect_ious_aligned = np.asarray([0.0714, 0.5000, 0.5000], dtype=np.float32)
    ...
    def test_box_iou_quadri_iof_cuda(self, device):
        ...
        np_expect_ious = np.asarray(
            [
                [0.1111, 1.0000, 0.0000],
                [0.0000, 1.0000, 0.0000],
                [0.0000, 0.0000, 1.0000],
            ],
            dtype=np.float32,
        )
        np_expect_ious_aligned = np.asarray([0.1111, 1.0000, 1.0000], dtype=np.float32)
