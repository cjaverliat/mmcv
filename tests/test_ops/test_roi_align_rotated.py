...
# yapf:disable
inputs = [
    ([[[[1.0, 2.0], [3.0, 4.0]]]], [[0.0, 0.5, 0.5, 1.0, 1.0, 0]]),
    ([[[[1.0, 2.0], [3.0, 4.0]]]], [[0.0, 0.5, 0.5, 1.0, 1.0, np.pi / 2]]),
    (
        [[[[1.0, 2.0], [3.0, 4.0]], [[4.0, 3.0], [2.0, 1.0]]]],
        [[0.0, 0.5, 0.5, 1.0, 1.0, 0]],
    ),
    (
        [
            [
    ...
    (
        ...
        [[0.0, 1.5, 1.5, 3.0, 3.0, np.pi / 2]],
    ),
]
outputs = [
    ([[[[1.0, 1.25], [1.5, 1.75]]]], [[[[3.0625, 0.4375], [0.4375, 0.0625]]]]),
    ([[[[1.5, 1], [1.75, 1.25]]]], [[[[3.0625, 0.4375], [0.4375, 0.0625]]]]),
    (
        [[[[1.0, 1.25], [1.5, 1.75]], [[4.0, 3.75], [3.5, 3.25]]]],
        [[[[3.0625, 0.4375], [0.4375, 0.0625]], [[3.0625, 0.4375], [0.4375, 0.0625]]]],
    ),
    (
...
def _test_roialign_rotated_gradcheck(device, dtype):
    ...
    for case in inputs:
        ...
        if torch.__version__ == "parrots":
            gradcheck(froipool, (x, rois), no_grads=[rois], delta=1e-5, pt_atol=1e-5)
        else:
...
def _test_roialign_rotated_allclose(device, dtype):
    ...
    for case, output in zip(inputs, outputs):
        ...
        output = roi_align_rotated(
            x, rois, (pool_h, pool_w), spatial_scale, sampling_ratio, True
        )
