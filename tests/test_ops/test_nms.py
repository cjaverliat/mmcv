...
class Testnms:
    ...
    def test_softnms_allclose(self):
        ...
        np_output = {
            "linear": {
                "dets": np.array(
                    [
                        [3.0, 6.0, 9.0, 11.0, 0.9],
                        [6.0, 3.0, 8.0, 7.0, 0.6],
                        [3.0, 7.0, 10.0, 12.0, 0.29024392],
                        [1.0, 4.0, 13.0, 7.0, 0.2],
                    ],
                    dtype=np.float32,
                ),
                "inds": np.array([1, 0, 2, 3], dtype=np.int64),
            },
            "gaussian": {
                "dets": np.array(
                    [
                        [3.0, 6.0, 9.0, 11.0, 0.9],
                        [6.0, 3.0, 8.0, 7.0, 0.59630775],
                        [3.0, 7.0, 10.0, 12.0, 0.35275510],
                        [1.0, 4.0, 13.0, 7.0, 0.18650459],
                    ],
                    dtype=np.float32,
                ),
                "inds": np.array([1, 0, 2, 3], dtype=np.int64),
            ...
            "naive": {
                ...
                "inds": np.array([1, 0, 3], dtype=np.int64),
            },
        }
        ...
        configs = [
            [0.3, 0.5, 0.01, "linear"],
            [0.3, 0.5, 0.01, "gaussian"],
            [0.3, 0.5, 0.01, "naive"],
        ]

        for iou, sig, mscore, m in configs:
            dets, inds = soft_nms(
                boxes, scores, iou_threshold=iou, sigma=sig, min_score=mscore, method=m
            )
    ...
    def test_nms_match(self):
        ...
        nms_keep_inds = nms(boxes.contiguous(), scores.contiguous(), iou_thr)[1]
    ...
    def test_batched_nms(self):
        ...
        nms_max_num = 100
        nms_cfg = dict(
            type="nms", iou_threshold=0.7, score_threshold=0.5, max_num=nms_max_num
        )
        ...
        assert torch.equal(keep, torch.from_numpy(results["keep"][:nms_max_num]))
