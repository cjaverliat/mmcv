...
class TestPad:
    ...
    def test_pad(self):
        ...
        trans = Pad(size=(2000, 2000), pad_val=dict(img=(12, 12, 12), seg=(10, 10, 10)))
        results = trans(copy.deepcopy(data_info))
        ...
        trans = Pad(
            pad_to_square=True, pad_val=dict(img=(12, 12, 12), seg=(10, 10, 10))
        )
        results = trans(copy.deepcopy(data_info))
...
class TestCenterCrop:
    ...
    def reset_results(results, original_img, gt_semantic_map):
        ...
        results["gt_bboxes"] = np.array([[0, 0, 210, 160], [200, 150, 400, 300]])
        results["gt_keypoints"] = np.array(
            [[[20, 50, 1]], [[200, 150, 1]], [[300, 225, 1]]]
        )
        return results
    ...
    def test_transform(self):
        ...
        assert (results["gt_seg_map"] == self.gt_semantic_map[38:262, 88:312]).all()
        assert np.equal(
            results["gt_bboxes"], np.array([[0, 0, 122, 122], [112, 112, 224, 224]])
        ).all()
        assert np.equal(
            results["gt_keypoints"],
            np.array([[[0, 12, 0]], [[112, 112, 1]], [[212, 187, 1]]]),
        ).all()

        # test CenterCrop when size is tuple
        transform = dict(type="CenterCrop", crop_size=(224, 224))
        center_crop_module = TRANSFORMS.build(transform)
        results = self.reset_results(results, self.original_img, self.gt_semantic_map)
        results = center_crop_module(results)
        assert results["img_shape"] == (224, 224)
        assert (results["img"] == self.original_img[38:262, 88:312, ...]).all()
        assert (results["gt_seg_map"] == self.gt_semantic_map[38:262, 88:312]).all()
        ...
        assert (results["gt_seg_map"] == self.gt_semantic_map[22:278, 88:312]).all()
        assert np.equal(
            results["gt_bboxes"], np.array([[0, 0, 122, 138], [112, 128, 224, 256]])
        ).all()
        assert np.equal(
            results["gt_keypoints"],
            np.array([[[0, 28, 0]], [[112, 128, 1]], [[212, 203, 1]]]),
        ).all()
        ...
        assert np.equal(
            results["gt_keypoints"],
            np.array([[[20, 50, 1]], [[200, 150, 1]], [[300, 225, 1]]]),
        ).all()

        # test CenterCrop when crop_size is larger than img.shape
        transform = dict(type="CenterCrop", crop_size=(img_width * 2, img_height * 2))
        center_crop_module = TRANSFORMS.build(transform)
        results = self.reset_results(results, self.original_img, self.gt_semantic_map)
        results = center_crop_module(results)
        assert results["img_shape"] == (300, 400)
        ...
        assert np.equal(
            results["gt_bboxes"], np.array([[0, 0, 110, 160], [100, 150, 200, 300]])
        ).all()
        assert np.equal(
            results["gt_keypoints"],
            np.array([[[0, 50, 0]], [[100, 150, 1]], [[200, 225, 0]]]),
        ).all()
        ...
        transform = dict(
            ...
            auto_pad=True,
            pad_cfg=dict(
                type="Pad", padding_mode="constant", pad_val=dict(img=13, seg=33)
            ),
        )
        center_crop_module = TRANSFORMS.build(transform)
        results = self.reset_results(results, self.original_img, self.gt_semantic_map)
        ...
        transform = dict(type="CenterCrop", crop_size=(img_width // 2, img_height))
        center_crop_module = TRANSFORMS.build(transform)
        results = self.reset_results(results, self.original_img, self.gt_semantic_map)
        results = center_crop_module(results)
        assert results["img_shape"] == (img_height, img_width // 2)
        assert (results["img"] == self.original_img[:, 100:300, ...]).all()
        assert (results["gt_seg_map"] == self.gt_semantic_map[:, 100:300]).all()
        assert np.equal(
            results["gt_bboxes"], np.array([[0, 0, 110, 160], [100, 150, 200, 300]])
        ).all()
        assert np.equal(
        ...
        transform = dict(type="CenterCrop", crop_size=(img_width, img_height // 2))
        center_crop_module = TRANSFORMS.build(transform)
        results = self.reset_results(results, self.original_img, self.gt_semantic_map)
        results = center_crop_module(results)
        assert results["img_shape"] == (img_height // 2, img_width)
        assert (results["img"] == self.original_img[75:225, ...]).all()
        assert (results["gt_seg_map"] == self.gt_semantic_map[75:225, ...]).all()
        assert np.equal(
            results["gt_bboxes"], np.array([[0, 0, 210, 85], [200, 75, 400, 150]])
        ).all()
        assert np.equal(
            results["gt_keypoints"],
            np.array([[[20, 0, 0]], [[200, 75, 1]], [[300, 150, 0]]]),
        ).all()
...
class TestRandomGrayscale:
    ...
    def test_transform(self):
        ...
        transform = dict(type="RandomGrayscale", prob=0.0)
        random_gray_scale_module = TRANSFORMS.build(transform)
        results["img"] = copy.deepcopy(self.img)
        img = random_gray_scale_module(results)["img"]
        ...
        transform = dict(type="RandomGrayscale", prob=1.0)
...
class TestRandomChoiceResize:
    ...
    def test_repr(self):
        # test repr
        transform = dict(type="RandomChoiceResize", scales=[(1333, 800), (1333, 600)])
    ...
    def test_random_multiscale_resize(self):
        ...
        self.reset_results(results)
        results = random_multiscale_resize(results)
        assert (results["img"].shape[1], results["img"].shape[0]) in _scale_choice
        ...
        assert np.equal(results["gt_bboxes"], np.array([[100, 75, 200, 150]])).all()
        ...
        results = random_multiscale_resize(results)
        assert results["img"].shape == (150, 200, 3)
        assert np.equal(results["gt_bboxes"], np.array([[100, 75, 300, 225]])).all()
...
class TestRandomFlip:
    ...
    def test_transform(self):
        ...
        assert (results_update["gt_bboxes"] == np.array([[124, 1, 224, 101]])).all()
        assert (results_update["gt_seg_map"] == np.array([[1, 0], [3, 2]])).all()

        # diagonal flip
        TRANSFORMS = RandomFlip([1.0], ["diagonal"])
        results_update = TRANSFORMS.transform(copy.deepcopy(results))
        assert (results_update["gt_bboxes"] == np.array([[124, 123, 224, 223]])).all()
        assert (results_update["gt_seg_map"] == np.array([[3, 2], [1, 0]])).all()

        # vertical flip
        TRANSFORMS = RandomFlip([1.0], ["vertical"])
        results_update = TRANSFORMS.transform(copy.deepcopy(results))
        assert (results_update["gt_bboxes"] == np.array([[0, 123, 100, 223]])).all()
        assert (results_update["gt_seg_map"] == np.array([[2, 3], [0, 1]])).all()
        ...
        TRANSFORMS = RandomFlip([1.0], ["horizontal"], swap_seg_labels=[[0, 1]])
        results_update = TRANSFORMS.transform(copy.deepcopy(results))
        assert (results_update["gt_seg_map"] == np.array([[0, 1], [3, 2]])).all()
        assert results_update["swap_seg_labels"] == [[0, 1]]

        TRANSFORMS = RandomFlip(0.0)
        results_update = TRANSFORMS.transform(copy.deepcopy(results))
        assert (results_update["gt_bboxes"] == np.array([[0, 1, 100, 101]])).all()
        assert (results_update["gt_seg_map"] == np.array([[0, 1], [2, 3]])).all()

        # flip direction is invalid in bbox flip
        with pytest.raises(ValueError):
            TRANSFORMS = RandomFlip(1.0)
            results_update = TRANSFORMS._flip_bbox(
                results["gt_bboxes"], (224, 224), "invalid"
            )
        ...
        with pytest.raises(AssertionError):
            ...
            results_update = TRANSFORMS._flip_seg_map(
                results["gt_seg_map"], "horizontal"
            )
...
class TestRandomResize:
    ...
    def test_transform(self):
        ...
        results_update = TRANSFORMS.transform(copy.deepcopy(results))
        assert results_update["scale"][0] >= 224 and results_update["scale"][0] <= 448
        assert results_update["scale"][1] >= 224 and results_update["scale"][1] <= 448
        ...
        TRANSFORMS = RandomResize(
            (224, 224), (1.0, 2.0), resize_type="Resize", keep_ratio=True
        )
        results_update = TRANSFORMS.transform(copy.deepcopy(results))
        ...
        # keep ratio is False
        TRANSFORMS = RandomResize(
            (224, 224), (1.0, 2.0), resize_type="Resize", keep_ratio=False
        )
        results_update = TRANSFORMS.transform(copy.deepcopy(results))
        ...
        results = {}
        TRANSFORMS = RandomResize(
            [(224, 448), (112, 224)], resize_type="Resize", keep_ratio=True
        )
        results_update = TRANSFORMS.transform(copy.deepcopy(results))
        assert results_update["scale"][1] >= 224 and results_update["scale"][1] <= 448
        assert results_update["scale"][0] >= 112 and results_update["scale"][0] <= 224
...
class TestTestTimeAug:
    ...
    def test_transform(self):
        ...
        target_results = [
            normalize.transform(
                flip1.transform(resize1.transform(copy.deepcopy(input_results)))
            ),
            normalize.transform(
                flip2.transform(resize1.transform(copy.deepcopy(input_results)))
            ),
            normalize.transform(
                flip1.transform(resize2.transform(copy.deepcopy(input_results)))
            ),
            normalize.transform(
                flip2.transform(resize2.transform(copy.deepcopy(input_results)))
            ),
        ]
