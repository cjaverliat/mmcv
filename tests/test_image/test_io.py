...
class TestIO:
    ...
    def test_imread(self):
        ...
        with pytest.raises(
            ValueError, match='"file_client_args" and "backend_args" cannot be set'
        ):
            mmcv.imread(
                self.img_path,
        ...
        img_cv2_color_bgr = mmcv.imread(self.img_path)
        with patch.object(
            PetrelBackend, "get", return_value=img_cv2_color_bgr
        ) as mock_method:
            img_cv2_color_bgr_petrel = mmcv.imread(self.s3_path, backend="cv2")
            img_cv2_color_bgr_petrel_with_args = mmcv.imread(
                self.s3_path, backend="cv2", file_client_args={"backend": "petrel"}
            )
            mock_method.assert_called()
            assert_array_equal(
                img_cv2_color_bgr_petrel, img_cv2_color_bgr_petrel_with_args
            )

            mock_method.reset_mock()

            img_cv2_color_bgr_petrel_with_args = mmcv.imread(
                self.s3_path, backend="cv2", backend_args={"backend": "petrel"}
            )
            mock_method.assert_called()
            assert_array_equal(
                img_cv2_color_bgr_petrel, img_cv2_color_bgr_petrel_with_args
        ...
        with patch.object(
            ...
            img_cv2_color_bgr_http_with_args = mmcv.imread(
                self.http_path, backend="cv2", backend_args={"backend": "http"}
            )
            mock_method.assert_called()
            assert_array_equal(img_cv2_color_bgr_http, img_cv2_color_bgr_http_with_args)
        ...
        img_pil_gray_alpha = mmcv.imread(self.gray_alpha_img_path, backend="pillow")
        ...
        img_turbojpeg_color_rgb = mmcv.imread(self.img_path, channel_order="rgb")
        ...
        img_cv2_exif_unchanged = mmcv.imread(self.exif_img_path, flag="unchanged")
        ...
        img_pil_color_ignore_exif = mmcv.imread(
            self.exif_img_path, backend="pillow", flag="color_ignore_orientation"
        )
        ...
        img_pil_grayscale_ignore_exif = mmcv.imread(
            self.exif_img_path, backend="pillow", flag="grayscale_ignore_orientation"
        )
    ...
    def test_imwrite(self):
        ...
        # test petrel client
        with patch.object(PetrelBackend, "put", return_value=None) as mock_method:
