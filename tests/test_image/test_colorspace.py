...
def test_bgr2gray():
    ...
    computed_gray = (
        in_img[:, :, 0] * 0.114 + in_img[:, :, 1] * 0.587 + in_img[:, :, 2] * 0.299
    )
...
def test_rgb2gray():
    ...
    computed_gray = (
        in_img[:, :, 0] * 0.299 + in_img[:, :, 1] * 0.587 + in_img[:, :, 2] * 0.114
    )
    assert_array_almost_equal(out_img, computed_gray, decimal=4)
...
def test_ycbcr2rgb():
    ...
    for i in range(in_img.shape[0]):
        for j in range(in_img.shape[1]):
            ...
            b = -276.836 + y * 0.00456621 * 255.0 + cb * 0.00791071 * 255
