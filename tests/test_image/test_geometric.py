...
class TestGeometric:
    ...
    def setup_class(cls):
        ...
        cls.img_pt = torch.from_numpy(cls.img)
    ...
    def test_imresize_pt(self):
        ...
        assert id(resized_img_dst) == id(resized_img)
        assert_array_equal(resized_img_dst, mmcv.imresize(self.img_pt, (1000, 600)))

        for mode in ["nearest", "bilinear", "bicubic"]:
            resized_img = mmcv.imresize(self.img_pt, (1000, 600), interpolation=mode)
            assert resized_img.shape == (600, 1000, 3)

        # Lanczos and area are not supported for torch.Tensor
        for mode in ["lanczos", "area"]:
            with pytest.raises(ValueError):
                mmcv.imresize(self.img_pt, (1000, 600), interpolation=mode)
