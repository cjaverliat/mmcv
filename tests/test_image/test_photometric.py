...
class TestPhotometric:
    ...
    def test_adjust_color(self, nb_rand_test=100):
        ...
        for _ in range(nb_rand_test):
            img = np.clip(np.random.normal(0, 1, (256, 256, 3)) * 260, 0, 255).astype(
                np.uint8
            )
    ...
    def test_adjust_brightness(self, nb_rand_test=100):
        ...
        assert_array_equal(mmcv.adjust_brightness(img, 1.0), img)
        # test case with factor 0.0
        assert_array_equal(mmcv.adjust_brightness(img, 0.0), np.zeros_like(img))
        # test adjust_brightness with randomly sampled images and factors.
        for _ in range(nb_rand_test):
            img = np.clip(
                np.random.uniform(0, 1, (1000, 1200, 3)) * 260, 0, 255
            ).astype(np.uint8)
            factor = np.random.uniform() + np.random.choice([0, 1])
            np.testing.assert_allclose(
                mmcv.adjust_brightness(img, factor).astype(np.int32),
                mmcv.adjust_brightness(img, factor, backend="pillow").astype(np.int32),
                rtol=0,
                atol=1,
            )
    ...
    def test_adjust_contrast(self, nb_rand_test=100):
        ...
        img = np.stack([img, img, img], axis=-1)
        # test case with factor 1.0
        assert_array_equal(mmcv.adjust_contrast(img, 1.0), img)
        # test case with factor 0.0
        assert_array_equal(
            mmcv.adjust_contrast(img, 0.0),
            mmcv.adjust_contrast(img, 0.0, backend="pillow"),
        )
        # test adjust_contrast with randomly sampled images and factors.
        for _ in range(nb_rand_test):
            img = np.clip(
                np.random.uniform(0, 1, (1200, 1000, 3)) * 260, 0, 255
            ).astype(np.uint8)
            factor = np.random.uniform() + np.random.choice([0, 1])
            ...
            np.testing.assert_allclose(
                ...
                mmcv.adjust_contrast(img, factor, backend="pillow").astype(np.int32),
                rtol=0,
                atol=1,
            )
    ...
    def test_auto_contrast(self, nb_rand_test=100):
        ...
        assert_array_equal(mmcv.auto_contrast(img, 10), _auto_contrast(img, 10))
        # test case with cut-off as float
        assert_array_equal(mmcv.auto_contrast(img, 12.5), _auto_contrast(img, 12.5))
        # test case with cut-off as tuple
        assert_array_equal(mmcv.auto_contrast(img, (10, 10)), _auto_contrast(img, 10))
        # test case with cut-off with sum over 100
        assert_array_equal(mmcv.auto_contrast(img, 60), _auto_contrast(img, 60))
    ...
    def test_adjust_sharpness(self, nb_rand_test=100):
        ...
        with pytest.raises(AssertionError):
            mmcv.adjust_sharpness(img, 1.0, kernel=1.0)
        # test case with invalid shape of kernel
        kernel = np.ones((3, 3, 3))
        with pytest.raises(AssertionError):
            mmcv.adjust_sharpness(img, 1.0, kernel=kernel)
        # test case with all-zero kernel, factor 0.0
        kernel = np.zeros((3, 3))
        assert_array_equal(
            mmcv.adjust_sharpness(img, 0.0, kernel=kernel), np.zeros_like(img)
        )

        # test case with factor 1.0
        assert_array_equal(mmcv.adjust_sharpness(img, 1.0), img)
        ...
        for _ in range(nb_rand_test):
            ...
            np.testing.assert_allclose(
                mmcv.adjust_sharpness(img, factor).astype(np.int32)[1:-1, 1:-1],
    ...
    def test_adjust_hue(self):
        ...
        # test pillow backend
        with pytest.raises(AssertionError):
            mmcv.adjust_hue(self.img.astype(np.float32), hue_factor=0, backend="pillow")
