...
def test_conv_module():
    ...
    efficient_conv = ConvModule(
        3, 8, 2, norm_cfg=dict(type="BN"), efficient_conv_bn_eval=True
    ).eval()
    plain_conv = ConvModule(
        3, 8, 2, norm_cfg=dict(type="BN"), efficient_conv_bn_eval=False
    ).eval()
    for efficient_param, plain_param in zip(
        efficient_conv.state_dict().values(), plain_conv.state_dict().values()
    ):
...
def test_bias():
    ...
    assert len(record) == 1
    assert (
        record[0].message.args[0] == "Unnecessary conv bias before batch/instance norm"
    )
