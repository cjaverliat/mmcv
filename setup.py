...
try:
    ...
    if torch.__version__ == "parrots":
        ...
        hasattr(torch, "is_musa_available") and torch.is_musa_available()
...
def choose_requirement(primary, secondary):
    """If some version of primary requirement installed, return primary, else return
    secondary."""
    try:
...
def parse_requirements(fname="requirements/runtime.txt", with_version=True):
    """Parse the package dependencies listed in a requirements file but strips specific
    ...
    if EXT_TYPE == "parrots":
        ...
        if os.getenv("MMCV_WITH_DIOPI", "0") == "1":
            ...
            if parse_version(torch.__version__) < parse_version("2.1.0"):
                op_files += glob.glob("./mmcv/ops/csrc/common/mps/*.mm") + glob.glob(
            ...
            try:
                ...
                extra_compile_args["cxx"] += [
                    "-I"
                    + importlib.util.find_spec("torch_npu").submodule_search_locations[
                        0
                    ]
...
def get_wheel_filename(name, version, local_label):
    ...
    py = get_python_tag()
    dist = name.replace("-", "_")
    return f"{dist}-{version}+{local_label}-{py}-{py}-" f"{get_platform_tag()}.whl"
        ...
        class CachedWheelsCommand(_bdist_wheel):
            ...
            def run(self):
                wheel_name = get_wheel_filename(
                    PACKAGE_NAME, get_version(), local_label
                )
                ...
                try:
                    ...
                    print(
                        f"mmcv: no matching prebuilt wheel ({e}); "
...
full_version = (
    get_version() if local_label is None else f"{get_version()}+{local_label}"
)
