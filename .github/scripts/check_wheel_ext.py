...
def main(dist_dir: str) -> int:
    ...
    if len(wheels) != 1:
        print(f'ERROR: expected exactly one wheel in {dist_dir!r}, '
              f'found {wheels}')
        return 1
    ...
    ext = [
        n for n in names
        if n.startswith('mmcv/_ext') and n.endswith(EXT_SUFFIXES)
    ]

    print(f'wheel: {os.path.basename(wheel)}')
    if not ext:
        print('ERROR: no compiled mmcv/_ext.(so|pyd) found in the wheel — '
